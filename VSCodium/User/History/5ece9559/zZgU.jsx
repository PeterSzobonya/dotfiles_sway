...
function JobCard({ job, openDetails, removeJob }) {
    ...
    return (
        <Card className="job-card" key={job.id} sx={{ maxWidth: 1100, width: 1100, margin: 3 }}>
            <CardHeader
                titleTypographyProps={{ align: 'left' }}
                title={job.position}
                action={
                    <Box sx={{ display: 'flex', gap: 1 }}>
                        <Button variant="outlined" color="primary" onClick={() => showJob(job)}>Megnéz</Button>
                        <Button variant="outlined" color="primary" >Szerkeszt</Button>
                        <Button variant="contained" color="error" onClick={() => deleteJob(job)}>Töröl</Button>
                    </Box>
                }
            />
            <CardContent>
                <div className='row' style={{ display: 'flex' }}>
                    <div className="col" style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        <div className="text">{createReadableType(job.type)}</div>
                    </div>
                    <div className="col" style={{ display: 'flex', alignItems: 'center' }}>
