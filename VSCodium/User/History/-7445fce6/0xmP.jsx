...
import {  useAddExperiencesMutation, useDeleteExperienceMutation, useGetExperiencesQuery, useUpdateExperienceMutation } from "../../redux/reducers/experience/experienceReducerApi";
...
function Profile() {
    ...
    const { data, refetch } = useGetExperiencesQuery(token, { refetchOnMountOrArgChange: true, skip: false });
    ...
    const handleRemoveClick = async (e, id) => {
        if (e.id !== null || e.id !== undefined) {
            try {
                const eId = e.id;
                const result = await deleteExperience({eId, token});
            } catch (e) {
                console.log(e);
            }
        }
